...
import { drawPoses } from './drawing';
import IMG_1 from './testBG1.jpg';
import IMG_2 from './testBG2.jpg';
import IMG_3 from './testBG3.jpg';
...
function setupGui(cameras) {
  ...
  gui.add(guiState, 'backgroundImage', ['A', 'B', 'C']).onChange((mode) => {
    if (mode === 'A') {
      bgImg.src = IMG_1;
    } else if ( mode === 'B') {
      bgImg.src = IMG_2;
  ...
  const estimateController =
      gui.add(guiState, 'estimate', ['custom', 'segmentation', 'partmap']);
...
function segmentBodyInRealTime() {
  ...
  async function bodySegmentationFrame() {
    ...
    switch (guiState.estimate) {
        case 'custom':
          multiPersonSegmentation = await estimateSegmentation();
          if (Array.isArray(multiPersonSegmentation) && multiPersonSegmentation.length > 0) {
            const ctx = canvas.getContext('2d');
            offCtx.clearRect(0, 0, 640, 480);
            offCtx.drawImage(video, 0, 0);
            multiPersonSegmentation.forEach(imageData => {
              const iData = offCtx.getImageData(0, 0, 640, 480);
              for (let i = 0; i < iData.height; i++) {
                for (let j = 0; j < iData.width; j++) {
                  let estIndex = ((i * iData.width) + j);
                  if (imageData.data[estIndex] > 0) {
                    iData.data[estIndex * 4 + 3] = 255;
                  } else {
                    iData.data[estIndex * 4 + 0] = 0;
                    iData.data[estIndex * 4 + 1] = 0;
                    iData.data[estIndex * 4 + 2] = 0;
                    iData.data[estIndex * 4 + 3] = 0;
                  }
                }
              }
